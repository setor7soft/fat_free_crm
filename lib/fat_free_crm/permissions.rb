    ...
    module InstanceMethods
        ...
        class_eval %Q{
          ...
          def #{model}_ids=(value)
            if access != "Shared"
              ...
              permissions_to_remove = Permission.where(["#{model}_id = ? and asset_id = ? and asset_type = ? ", self.#{model}_ids - value, self.id, self.class ])
      ...
      def remove_permissions
        ...
        if self.id && self.class
          permissions_to_remove = Permission.where(asset_id: self.id, asset_type: self.class).load
        else
